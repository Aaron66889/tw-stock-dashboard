...
## 16.8.5 — Yahoo台股四檔ETF直接即時價
四檔ETF現價來源固定為 Yahoo Taiwan 對應商品代號：
- 0050.TW
- 0056.TW
- 00878.TW
- 00919.TW

四檔分開請求，單檔失敗不阻塞其他三檔；盤中每2秒刷新。此修改只作用於四檔ETF現價，不修改買點模型、歷史、回測、成分股或驗證邏輯。

## 16.8.6 — Yahoo頁面現價對齊
- 修正 16.8.5 使用 Yahoo chart 最後一根 5 分K close，造成可落後 Yahoo 台股報價頁。
- 現價現在優先採 Yahoo `meta.regularMarketPrice`，時間採 `meta.regularMarketTime`。
...
## 16.8.8 — 單一ETF現價來源 + 買點觸發 + 模型買入同步持股
- 四檔ETF現價唯一主人為 Yahoo ETF live；`/api/market` 不得再覆蓋四檔ETF現價。
- 每次 Yahoo ETF tick 都用同一個現價執行 Layer 1/2/3 狀態機。
- 畫面狀態改為顯示目前最深一層已觸發/形成中的層級，不再永遠只看第一層。
- 「記錄模型買入」會同步增加實際持股並用加權成本重算均價。
- 不修改 server 端買點公式、歷史回測、成分股、完整歷史邏輯。

## 16.8.9 三層燈號同步（最小修改）
首頁、ETF買點、ETF詳細頁：第一/理想/強力三層皆為觸價橘燈、正式確認紅燈。正式Gate與買點公式不變。
一次性補登0050既有模型交易20股@105.40：只有交易紀錄存在且持股仍是3150股@77.37時才補成3170股並重算均價，避免重複。
...
## 16.8.13 Yahoo台灣即時價唯一基準
- 撤回16.8.12的Yahoo Finance API主來源；四檔ETF現價重新以Yahoo台灣報價頁為唯一基準。
- 後端不再每次同時抓4個完整頁面。改成每1.2秒輪抓1檔，約4.8秒完成一輪，降低Yahoo/Render限流與卡死風險。
- `/api/etf-live` 只讀已更新快取；冷啟動缺資料時才補抓。
- 前端仍每5秒讀取一次，價格上漲閃紅、下跌閃綠約0.9秒後恢復。
- 模型、Score、Gate、買點、燈號、持股、歷史、回測均未修改。

## 16.8.14 抓取時間顯示 ONLY
- 現價下方不再顯示Yahoo成交時間；改顯示瀏覽器成功取得 `/api/etf-live` 回應的抓取時間。
- 每次成功抓到四檔ETF資料，抓取時間都會更新，即使價格剛好沒有變動。
- 這個時間只代表「網站有成功抓資料」，不代表市場一定有新成交。
- 即時來源、模型、Score、Gate、買點、燈號、持股、歷史、回測都未修改。

## 16.8.15 抓取心跳
- 現價下方改成「抓取 HH:mm:ss｜#次數」。
- 只要瀏覽器成功收到 `/api/etf-live` 回應，就立即更新時間與次數，不依賴股價是否變動，也不依賴Yahoo成交時間。
- 暫停紅/綠閃爍驗證，先把「到底有沒有持續抓取」這件事看清楚。
- 即時來源、模型、Score、Gate、買點、持股、歷史、回測均未修改。

## 16.8.16 TWSE MIS 即時價
- 0050 / 0056 / 00878 / 00919 即時現價改走 TWSE MIS，與大盤/一般台股即時行情使用同一來源。
- Yahoo 完全退出四檔ETF即時價路徑。
- 批次抓四檔，若有單檔缺值再單獨補抓。
...
## 16.8.21 Anue 鉅亨即時行情主源
- 四檔ETF改由 Anue 鉅亨公開即時報價 API 作為主來源，一次批次抓 0050/0056/00878/00919。
- endpoint 無需登入/API Key；欄位使用 200010=代碼、200026=現價、200031=昨收。
- TWSE MIS 僅補 Anue 缺少的代碼，不得覆蓋 Anue 成功行情。
- 前端取消瀏覽器跨網域直抓，電腦與手機都只呼叫本站 `/api/etf-live`，因此行為一致。
- UI 會顯示「鉅亨即時」；若看到「TWSE備援」代表 Anue 該次沒有成功。
- 模型、Score、Gate、買點、持股、歷史、回測未修改。

## 16.8.22 成分股即時行情 only
- 僅修改 `quoteCodes()`：成分股盤中現價/昨收改以已驗證成功的 Anue 鉅亨批次行情為主源。
- 每批40檔；四檔ETF成分股共用同一報價函式。
- 鉅亨缺碼時才由原TWSE MIS補缺；仍缺才使用原官方日資料備援。
- 成分股名單與權重來源完全不動。
- `constituentHealth()` 計分公式完全不動，只會收到更新後的盤中行情。
- 16.8.21 ETF即時行情 `liveEtf4()` 完全不動。
- 前端 app.js byte-identical。
- 模型、Score、Gate、買點、持股、歷史、回測完全不動。

## 16.8.23 成分股即時現價刷新
- 保留16.8.22的鉅亨成分股行情 `quoteCodes()` 完全不動。
- 成分健康行情快取由45秒縮短為8秒，避免模型長時間吃舊成分股行情。
- 成分股頁面在開啟期間每10秒自動重新抓取/重算，不需手動切頁。
- 前十大熱力卡新增「現價」；其餘成分股表格原本已有現價欄，現在會隨10秒刷新更新。
- 16.8.21 ETF即時行情 `liveEtf4()` 完全不動。
- 模型公式、Score、Gate、持股、歷史、回測不動。

## 16.8.24 大盤＋台指期鉅亨化
- 加權指數 primary 改為鉅亨 `TWS:TSE01:INDEX`。
- 台積電環境行情同步優先鉅亨。
- 台指期/夜盤 primary 改讀鉅亨 TXF quote service；若鉅亨 quote schema/代碼未回傳，保留原 Yahoo WTX& 作 fallback，不讓夜盤直接斷線。
- ETF `liveEtf4()`、成分股 `quoteCodes()` 完全不動。
- Score、Gate、三層價格、持股、歷史、回測不動。

## 16.8.25 ETF 現價紅漲綠跌
- ETF 買點卡與 ETF 詳情的「現價」：高於昨收顯示紅色、低於昨收顯示綠色、平盤維持原色。
- 僅改前端顏色；16.8.24 大盤/台指期、ETF/成分股鉅亨行情、模型、Gate、歷史、回測全部不動。

## 16.8.26 ETF 現價＋今日漲跌幅
- ETF 現價旁新增「相對昨收的今日漲跌幅」，例如 `106.25 +0.71%`。
- 上漲維持紅色、下跌維持綠色、平盤原色。
- 僅改前端顯示與 build marker；行情、模型、Gate、歷史、回測不動。

## 16.8.27 驗證中心 #2 / #3 / #37
- #2 改為配合鉅亨 TXF：以「最新成交－參考價」在驗證端自行重算點數與百分比；鉅亨來源成功才 PASS，若走備援則 PARTIAL。
- #3 改為配合鉅亨 TXF：確認參考價、今晚最高與距高點是獨立欄位；鉅亨來源成功且欄位完整才 PASS。
- #37：0050 已接受 2009 起的長期完整可回測樣本；只要 `backtestReadyPass + longSamplePass`，驗證狀態直接視為 READY，不再卡 `LONG_SAMPLE_READY`。
- 僅修改驗證判定與 build marker；行情抓取、ETF/成分股、大盤/夜盤來源、模型、Gate、歷史資料內容、回測公式全部不動。

## 16.8.28 模型實戰淨損益 ONLY
- 只修改「我的模型實戰成績」損益口徑。
...
## R3.38 / 16.8.66：正式買點即時歸零＋持股點評快速化
- 任一層正式確認時，模型「連續未正式確認」立即歸零，不再等隔日換日。
- 重新載入後會從今日 triggeredAt、買點歷史與「模型買入」實戰紀錄校正顯示，避免 0050／00878 已有正式訊號卻仍顯示 5 日。
- 自主買入仍與模型訊號分開，不用個人主觀交易污染模型參與率。
- 「距上次實際買進」卡片只顯示實際買入，不再把「模型無正式買點 N 日」混在同一格造成語意衝突；模型 streak 移到模型參與率卡片獨立顯示。
- `/api/portfolio-commentary` 改優先使用瀏覽器快照與伺服器既有快取，不再為點評同步重跑完整 buyModel。
- 點評 API 若仍逾時，前端立即用既有持股、行情與模型狀態產生備援點評，不再整頁空白。
