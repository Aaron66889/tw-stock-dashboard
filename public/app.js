...
const DIVIDEND_LEDGER_VERSION='fubon-statement-aggregate-2026-09-22';
const DIVIDEND_SEED={
 '0050':2954,
 '0056':3836,
 '00878':15645,
...
function loadHoldings(){try{const x=JSON.parse(localStorage.getItem('twStockHoldingsV12'));const a=Array.isArray(x)?x:DEFAULT_H;return a.map(normalizeHoldingRecord)}catch{return DEFAULT_H.map(normalizeHoldingRecord)}}
...
function tradingSessionsBetween(fromDay,toDay){const ds=Array.isArray(lastBuy?.tradingDates)?lastBuy.tradingDates:[];return ds.filter(d=>d>=fromDay&&d<toDay).length}
function resetDaily(){
 const d=dayKey();if(STATE.day===d)return;const old=STATE,nextNo={...(STATE.noSignalDays||{})};
 if(old.day){
  // Advance the streak only by actual exchange dates supplied by the historical calendar. Weekends/holidays do not count.
  const sessions=tradingSessionsBetween(old.day,d);
  if(sessions>0)ETF.forEach(c=>{const triggered=old.models?.[c]?.layers?.some(x=>x.triggeredAt);nextNo[c]=triggered?Math.max(0,sessions-1):(Number(nextNo[c]||0)+sessions)});
 }
 STATE={day:d,models:{},noSignalDays:nextNo};saveJSON('v124_state',STATE)
}
...
function layerTouchState(L,px){
 ...
 if(L.touchedAt||L.triggeredAt||L.forming||L.confirmCount>0)return'TOUCHED';
 return'IDLE';
}
...
function chaseLockText(cal){const st=cal?.lockStatus;return{LOCKED:'已鎖定',CARRY_FORWARD:'沿用前次鎖定',WAIT_HISTORY:'歷史建立中',PRELOCK_CARRY:'08:30前沿用',PRELOCK_PREVIEW:'08:30前預覽',PRELOCK_FALLBACK:'08:30前等待',FALLBACK_LOCKED:'保守值鎖定'}[st]||st||'—'}
...
async function loadServerPreopen(){
 try{
  const d=await get('/api/preopen-snapshot',6000),s=d?.snapshot;
  if(!s||s.day!==dayKey())return;
...
function initShadowModel(code,r){const m=initModel(code,r);m.confirmationVersion=SHADOW_CONFIRM_VERSION;m.policyId=r?.shadowPolicy?.selectedId||'baseline';m.policyQ=r?.shadowPolicy?.q||null;m.criticalStatus=r?.criticalGate?.status||'BLOCKED';return m}
function updateShadowOne(code,r){
 if(!r||r.error)return;const calibrationVersion=(r.firstLayerCalibration?.version||'legacy')+'|shadow|'+(r?.shadowPolicy?.selectedId||'baseline'),rules=shadowRules(r);let m=SHADOW_STATE.models[code];if(!m||m.calibrationVersion!==calibrationVersion)m=initShadowModel(code,r);m.calibrationVersion=calibrationVersion;m.confirmationVersion=SHADOW_CONFIRM_VERSION;m.layers.forEach(normalizeLayerRuntime);
...
function shadowPanel(c){const s=shadowStatusFor(c),o=statusFor(c);if(!s||!o)return'';const d=(center(s.m.layers[0].zone)-center(o.m.layers[0].zone))/Math.max(.01,center(o.m.layers[0].zone))*100,gate=s.r?.criticalGate?.status||'BLOCKED',pol=s.r?.shadowPolicy||{},rule=shadowRules(s.r)[0],state=stageText(s.activeStatus,s.activeLayer+1),replayN=(SHADOW_LOG||[]).filter(x=>x.day===dayKey()&&x.code===c).length;return`<div class="shadowbox"><div class="row"><div><b>Shadow 新模型</b><small>僅比較，不影響正式買進訊號</small></div><span class="pill ${gate==='PASS'?'live':'warn'}">${gate==='PASS'?'Critical PASS':'Critical '+gate}</span></div><div class="shadowgrid"><span>① ${ztxt(s.m.layers[0].zone)}</span><span>② ${ztxt(s.m.layers[1].zone)}</span><span>③ ${ztxt(s.m.layers[2].zone)}</span></div><small>${state}｜L1差異 ${d>=0?'+':''}${d.toFixed(2)}%｜OOS ${pol.selectedId||'baseline'} / WF ${pol.wfCount||0}｜L1分數≥${rule.minScore} / ${rule.cycles}筆｜時間制漂移｜盤中Replay ${replayN}筆</small>${confirmSnapshotText(s.m.layers[0])?`<small>${confirmSnapshotText(s.m.layers[0])}</small>`:''}</div>`}
...
function etfDailyChangeText(code,px){
 ...
 const d=last-prev,p=d/prev*100;
...
function renderCommentary(d){
 ...
 if(d.empty){$('commentarySummary').innerHTML=(d.summary||[]).map(x=>`<div class="commentarySummaryLine">${x}</div>`).join('');$('commentaryOverall').textContent='請先在「我的持股」建立持股，這裡才會產生每日總評。';$('commentaryStyle').textContent='尚無持股可分析';$('commentaryPortfolio').innerHTML='';$('commentaryFocus').innerHTML='';$('commentaryMarket').textContent='—';$('commentaryAction').textContent='—';$('commentaryConclusion').textContent='—';$('commentaryDisclaimer').textContent=d.disclaimer||'';return}
 $('commentarySummary').innerHTML=(d.summary||[]).map(x=>`<div class="commentarySummaryLine">${x}</div>`).join('');
 $('commentaryOverall').innerHTML=(d.overallCommentary||[]).map(x=>`<p>${x}</p>`).join('')||'<p>目前資料不足，待行情補齊後自動更新。</p>';
...
function renderHoldings(){
 let val=0,cost=0,dividendTotal=0,pendingDividendTotal=0;
 ...
 let cards='';
 for(const h of H){
  const q=lastLive?.quotes?.[h.t],px=q?.last,v=px?px*h.s:null,c=h.c*h.s,p=v!=null?v-c:null,r=p!=null&&c?p/c*100:null;
...
function renderDetailBuy(){const x=statusFor(selectedETF);if(!x)return;$('detailBuy').innerHTML=`<div class="buygrid"><div class="buybox"><span class="k">現價</span><b class="${etfPriceClass(selectedETF,x.px)}">${fmt(x.px)} <span class="etf-daily-pct">${etfDailyChangeText(selectedETF,x.px)}</span></b><small>${etfFetchStamp()}${etfExchangeStamp(selectedETF)}</small></div><div class="buybox ${layerBoxClass(x.m.layers[0],x.px,'first')}"><span class="k">${layerBoxLabel('第一買點',x.m.layers[0],x.px)}</span><b>${ztxt(x.m.layers[0].zone)}</b></div><div class="buybox ${layerBoxClass(x.m.layers[1],x.px)}"><span class="k">${layerBoxLabel('理想買點',x.m.layers[1],x.px)}</span><b>${ztxt(x.m.layers[1].zone)}</b></div><div class="buybox ${layerBoxClass(x.m.layers[2],x.px)}"><span class="k">${layerBoxLabel('強力買點',x.m.layers[2],x.px)}</span><b>${ztxt(x.m.layers[2].zone)}</b></div></div><div class="reading">${explain(selectedETF,x)}<br><b>決策拆解：</b>${decisionTransparency(x)}<br><b>第一層可觸及性：</b>${reachabilityText(x)}</div><button class="btn primary" style="margin-top:8px" onclick="openModelTrade('${selectedETF}',${x.activeLayer+1})">我照模型買了｜記錄實戰</button> <button class="btn" style="margin-top:8px" onclick="openManualTrade('${selectedETF}')">我自己買了｜未觸發模型</button>`;renderHistory(selectedETF)}
...
function clientValidation(base){const c={...(base||{})};const now=taipeiNow(),m=now.getHours()*60+now.getMinutes(),key=dayKey(),p=PREOPEN.day===key?(PREOPEN.locked||PREOPEN.draft):null;
 const server5=c[5];
 c[5]=server5?.status==='PASS'?server5:(p?{status:PREOPEN.locked?'PASS':'PARTIAL',evidence:PREOPEN.locked?(p.server?'Server今日盤前版本已鎖定':'今日盤前版本已鎖定'):'08:57盤前估算中',updatedAt:p.at}:server5||{status:'WAIT',evidence:'今日尚未捕捉真實08:57盤前版本'});
 ...
 const backend1=c[1];
 c[1]=(backend1?.status==='PASS')?{...backend1,evidence:backend1.evidence+'｜前端資料載入後即時重驗'}:{status:legacyOk===legacyClient.length?'PASS':legacyOk?'PARTIAL':'FAIL',evidence:`V12.3核心 ${legacyOk}/${legacyClient.length}｜`+legacyClient.map(([n,ok])=>`${n}:${ok?'OK':'FAIL'}`).join('｜')};
...
async function syncHoldingsCloud(force=false){
 ...
 try{
  ...
  if(!d.initialized){
   ...
   H=(Array.isArray(d.holdings)?d.holdings:[]).map(normalizeHoldingRecord);
...
function fillTradeSnapshot(code,x,manual=false){
 ...
 const state=x.r.noBuyToday?'今日暫無合理買點':stageText(x.activeStatus,x.activeLayer+1),sx=shadowStatusFor(code),shadowLine=sx?`<br><b>Shadow：</b>① ${ztxt(sx.m.layers[0].zone)}　② ${ztxt(sx.m.layers[1].zone)}　③ ${ztxt(sx.m.layers[2].zone)}｜${sx.r?.criticalGate?.status||'BLOCKED'}｜${stageText(sx.activeStatus,sx.activeLayer+1)}`:'';
 $('mtSnapshot').innerHTML=`<b>當下模型快照：</b>① ${ztxt(x.m.layers[0].zone)}　② ${ztxt(x.m.layers[1].zone)}　③ ${ztxt(x.m.layers[2].zone)}<br>分數 ${x.r.score}/100｜防追高 ${x.r.chaseRisk}/100｜環境 ${x.r.environmentScore.toFixed(0)}｜成分健康 ${x.r.health?.usable?x.r.health.score+'/100':'未計分'}<br>模型狀態：${state}${gap==null?'':`｜現價較第一買點上緣 ${gap>=0?'+':''}${gap.toFixed(2)}%`}${shadowLine}${manual?'<br><b>本筆將標記為「自主買入」，不計入模型買入績效。</b>':''}`;
}
...
function tradeDiagnosticLabel(t){
 const p=t?.perf||{},m=modelNetMetrics(t),parts=[];
 if(t.tradeType==='manual')parts.push('自主買入');else parts.push(`模型L${t.layer||'—'}`);
 if(Number.isFinite(p.maePct)&&p.maePct<=-1)parts.push('進場後曾有較深回撤');
 if(t.layer===1&&p.reachedLayer2)parts.push('L1後續曾到L2，需檢查是否偏早');
...
function performanceDiagnosticPayload(){
 const modelTrades=MODEL_TRADES.filter(t=>t.tradeType!=='manual'),byLayer={};
 for(const layer of [1,2,3]){
  const a=modelTrades.filter(t=>Number(t.layer)===layer),mae=a.map(t=>Number(t.perf?.maePct)).filter(Number.isFinite),mfe=a.map(t=>Number(t.perf?.mfePct)).filter(Number.isFinite),rets=a.map(modelNetMetrics).map(x=>x.ret).filter(Number.isFinite),bdelta=a.map(t=>Number(t.perf?.benchmarkDeltaCurrentPct)).filter(Number.isFinite);
 ...
 return{generatedAt:new Date().toISOString(),day:dayKey(),build:CLIENT_BUILD,confirmationRules:LAYER_CONFIRM_RULES,summary:{total:MODEL_TRADES.length,model:modelTrades.length,manual:MODEL_TRADES.filter(t=>t.tradeType==='manual').length,byLayer},trades:MODEL_TRADES.map(t=>{const p=t.perf||{},m=modelNetMetrics(t),bench=p.benchmark||{};return{id:t.id,code:t.code,name:t.name,tradeType:t.tradeType||'model',layer:t.layer,entryAt:t.entryAt,entryDate:t.entryDate,entryPrice:t.entryPrice,shares:t.shares,exitAt:t.exitAt||null,exitPrice:t.exitPrice||null,build:t.snapshot?.build||null,confirmationVersion:t.snapshot?.confirmationVersion||null,confirmationRule:t.snapshot?.confirmationRule||null,score:t.snapshot?.score??null,chaseRisk:t.snapshot?.chaseRisk??null,chaseWarningThreshold:t.snapshot?.chaseWarningThreshold??null,chaseHardThreshold:t.snapshot?.chaseHardThreshold??null,chaseCalibrationLockedDate:t.snapshot?.chaseCalibrationLockedDate??null,chaseCalibrationObservations:t.snapshot?.chaseCalibrationObservations??null,chaseCalibrationWalkForwardCount:t.snapshot?.chaseCalibrationWalkForwardCount??null,environmentScore:t.snapshot?.environmentScore??null,healthScore:t.snapshot?.health?.score??null,healthDivergence:t.snapshot?.health?.divergence??null,modelStatus:t.snapshot?.modelStatusText||null,zones:t.snapshot?.zones||null,shadow:t.snapshot?.shadow||null,currentPrice:p.currentPrice??null,netPnL:m.net,netReturnPct:m.ret,grossPnL:m.gross,estimatedCost:m.cost,horizon:p.horizon||null,maePct:p.maePct??null,maeDate:p.maeDate??null,mfePct:p.mfePct??null,mfeDate:p.mfeDate??null,reachedLayer2:!!p.reachedLayer2,reachedLayer3:!!p.reachedLayer3,chaseEntry:p.chaseEntry??null,benchmarkEntryMode:bench.entryMode||null,benchmarkEntryPrice:bench.entryPrice??null,benchmarkCurrentReturnPct:bench.currentReturnPct??null,benchmarkDeltaCurrentPct:p.benchmarkDeltaCurrentPct??null,benchmarkHorizon:bench.horizon||null,diagnosis:tradeDiagnosticLabel(t)} })};
}
