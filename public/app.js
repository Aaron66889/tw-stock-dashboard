...
function initModel(code,r){
 const mk=z=>({zone:JSON.parse(JSON.stringify(z)),samples:[],priceSamples:[],confirmed:false,forming:false,fastPass:false,invalid:false,confirmCount:0,badCount:0,triggeredAt:null,touchedAt:null,touchMin:null,confirmationReason:null});
 return{layers:[mk(r.raw.first),mk(r.raw.second),mk(r.raw.third)],calibrationVersion:r.firstLayerCalibration?.version||'legacy',confirmationVersion:CONFIRM_RULE_VERSION,prevEnv:r.environmentScore,prevHealth:r.health?.score??null,lastPrice:r.price,lastAt:new Date().toISOString()}
}
function updateOne(code,r){
 ...
 m.layers.forEach((L,i)=>{const z=L.zone,priceReached=px<=z.high,sd=L.samples.length>=4?Math.sqrt(L.samples.reduce((s,v)=>s+(v-L.samples.reduce((a,b)=>a+b,0)/L.samples.length)**2,0)/L.samples.length):999,zoneStable=sd<=atr*.08,rule=LAYER_CONFIRM_RULES[i]||LAYER_CONFIRM_RULES[0];
  if(priceReached){L.touchedAt=L.touchedAt||nowIso;L.touchMin=Number.isFinite(L.touchMin)?Math.min(L.touchMin,px):px;L.priceSamples.push(px);L.priceSamples=L.priceSamples.slice(-6)}
  ...
  // but still require two model cycles and no hard/soft veto. Buy-point prices themselves are unchanged.
  ...
  if(confirmable){L.confirmCount++;L.forming=true;L.confirmationReason=`${rule.name}｜分數≥${rule.minScore}｜${i===0?'價格止跌/反彈':'深回檔未持續加速'}｜Gate PASS`}
  else if((priceReached||L.touchedAt||Math.abs(px-center(z))<=atr*.28)&&zoneStable){L.forming=true;L.confirmCount=Math.max(0,L.confirmCount-1);L.confirmationReason=priceReached?`${rule.name}等待中｜需分數≥${rule.minScore}${i===0?'＋價格止跌':'＋連續深回檔確認'}`:L.confirmationReason}
  else{L.forming=false;L.confirmCount=Math.max(0,L.confirmCount-1)}
  if(L.confirmCount>=rule.cycles){L.confirmed=true;L.triggeredAt=L.triggeredAt||nowIso;addEvent(`${code} 第${i+1}層正式確認：${ztxt(z)}，現價${px.toFixed(2)}｜${L.confirmationReason}`,'buy')}
 });
 ...
 m.prevEnv=r.environmentScore;m.prevHealth=healthNow;m.lastPrice=px;m.lastAt=nowIso;STATE.models[code]=m;appendHistory(code,r,m,px)
}
