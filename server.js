...
const VERSION='V12.4';
const BUILD='16.8.47-DATA-CONSISTENCY';
...
async function constituentHealth(code){
 return cached('health:r328:'+code,8000,async()=>{
...
async function buyModel(){
 ...
 const [ld,etfLd,ctx,ovs,nf,harr,hhealth]=await Promise.all([
  ...
  deadline(liveEtf4().catch(e=>({ok:false,quotes:{},error:e.message,fetchedAt:null})),7000,{ok:false,quotes:{},error:'ETF live deadline exceeded',fetchedAt:null}),
 ...
 const marketAt=ld.fetchedAt?Date.parse(ld.fetchedAt):0,etfAt=etfLd?.fetchedAt?Date.parse(etfLd.fetchedAt):0;
 const marketFresh=!!marketAt&&Date.now()-marketAt<90000&&(!twMarketOpenNow()||ld.realtime!==false);
 const etfQuotesOk=ETF.every(c=>Number(etfLd?.quotes?.[c]?.last)>0&&Number(etfLd?.quotes?.[c]?.prevClose)>0);
